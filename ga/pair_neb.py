...
def neigh_dis(atoms1, atoms2, metal="Cu", distance_limit=4):
    ...
    metal_index = [i for i in range(len(chemical_symbols1)) if metal in chemical_symbols2[i]]
...
def load_config():
    ...
    parser = argparse.ArgumentParser(description="Structure processing with configurable parameters.")
    parser.add_argument('--metal', type=str, default='Cu', help="Metal type to look for in atoms (default: Cu)")
    parser.add_argument('--distance_limit', type=float, default=4.0, help="Distance limit for atoms to be considered (default: 4.0)")
...
if __name__ == "__main__":
    ...
    distance_limit = args.distance_limit  # Set the distance limit from command line argument
    ...
    for i in range(len(atoms)-1):
        for j in range(i+1, len(atoms)):
            atoms1 = atoms[i]
            atoms2 = atoms[j]
            atoms1, atoms2 = neigh_dis(atoms1, atoms2, metal=metal, distance_limit=distance_limit)
