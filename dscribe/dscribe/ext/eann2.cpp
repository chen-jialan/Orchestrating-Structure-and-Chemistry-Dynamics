...
inline double EANN::computeCutoff(double r_ij)
{
    if (r_ij <= rCut)
    {
        double result = 0.5*(cos(r_ij*PI/rCut)+1);
        return result;
    }
    else
    {
...
{
    double result = exp(-alpha*pow(r_ij-rs,2));
    // cout << "gaussian_fuc: " << result << endl;
    return result;
}

inline double EANN::costheta(double r_ij, double r_ik, double r_jk)
{
    if (r_ij <= rCut && r_ik <= rCut && abs(r_ik) < r_ij+r_ik)
    {
        ...
        )
{
    ...
    for (int &i : indices)
    {
        ...
        int size_neigh = i_neighbours.size();
        for (const int &j : i_neighbours)
        {
            ...
            if (size_neigh >= 2)
            {
                for (const int &k : i_neighbours)
                {
                    ...
                    {
                        //cout << "costheta_i_j_k: " << costheta_i_j_k << endl;
