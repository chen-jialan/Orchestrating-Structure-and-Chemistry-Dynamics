...
PYBIND11_MODULE(ext, m) {
    ...
    // EANN
    py::class_<EANN>(m, "EANNWrapper")
        .def(py::init<double, int, int, vector<int>, double, vector<double>, vector<vector<double>> > ())
        .def(py::init<>())
        .def("create", &EANN::create)
        .def_property("r_cut", &EANN::getRCut, &EANN::setRCut)
        .def_property("atomic_numbers", &EANN::getAtomicNumbers, &EANN::setAtomicNumbers)
        .def_property("nwave", &EANN::getnwave, &EANN::setnwave)
        .def_property("alpha", &EANN::getalpha, &EANN::setalpha)
        .def_property("orbital", &EANN::getnorbital, &EANN::setorbital)
        .def_property("rs", &EANN::getrs, &EANN::setrs)
        .def_property("c", &EANN::getc, &EANN::setc)
        .def(py::pickle(
            [](const EANN &p) {
                return py::make_tuple(p.rCut, p.nwave, p.orbital, p.atomicNumbers, p.alpha, p.rs, p.c);
            },
            [](py::tuple t) {
                if (t.size() != 7)
                    throw std::runtime_error("Invalid state!");
                EANN p(
                    ...
                    t[3].cast<vector<int> >(),
                    t[4].cast<double>(),
                    t[5].cast<vector<double> >(),
                    t[6].cast<vector<vector<double> > >()
                );
                return p;
            }
        ));
